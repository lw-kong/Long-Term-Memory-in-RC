function [validation_performance,W_in,res_net,W_out,t_validate,x_real,x_validate] = ...
    func_STP_train_NullDim_50_GPU_beta(udata,tp_train_set,dim_set,flag,...
...
W_out= y_reg *r_reg'/tp_length/(r_reg*r_reg'+beta*eye(n));
...
for tp_i = 1:tp_length
    ...
    %udata_g = gpuArray(udata(tp_i,:,:));
    
    dim_system = dim_set(tp_i);
    
    r_g = gpuArray(r_end(:,tp_i));
    u_g = zeros(dim+tp_dim,1,'gpuArray');
    u_g(1:dim_system) = udata(tp_i,train_length+1,1:dim_system); % first drive
    %u_g(dim+1:end) = udata(tp_i,train_length+1,dim+1:end);
    for t_i = 1:validate_length
        u_g(dim+1:end) = udata(tp_i,train_length+t_i,dim+1:end); %tp
        r_g = (1-a_g) * r_g + a_g * tanh(res_net_g*r_g + W_in_g*u_g);
        r_out = r_g;
        r_out(2:2:end) = r_out(2:2:end).^2; %even number -> squared
        predict_y = W_out_g * r_out;
        validate_predict_y_set(tp_i,t_i,:) = predict_y;
        u_g(1:dim_system) = predict_y(1:dim_system);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
