function [validation_performance,W_in,res_net,W_out,t_validate,x_real,x_validate] = ...
    func_STP_train_NullDim_61(udata,tp_train_set,dim_set,flag,...
    W_in_type,res_net_type,validation_type,print_progress_type,batch_size)
...
% _60
% YR & RR matrices that can significantly reduce the required memory when
% the number of states & RC sizes are large,
% but also make the echoing, and possibly the entire process slower
% because now we have to do these large matrcies multiplication for more
% times.
...
for batch_i = 1:tp_length/batch_size
    YR_temp = zeros(batch_size,dim,n);
    RR_temp = zeros(batch_size,n,n);
    r_end_temp = zeros(n,batch_size);
    udata_batch = udata(((batch_i-1)*batch_size+1):(batch_i*batch_size),:,:);
    parfor batch_j = 1:batch_size
        tp_i = (batch_i-1)*batch_size + batch_j;
        train_x = zeros(train_length,dim+tp_dim);
        train_y = zeros(train_length,dim+tp_dim);
        train_x(:,:)=udata_batch(batch_j,1:train_length,:);
        train_y(:,:)=udata_batch(batch_j,2:train_length+1,:);
        train_x = train_x';
        train_y = train_y';
        ...
        r_end_temp(:,batch_j) = r_all(:,end); % n * 1

        YR_temp(batch_j,:,:) = train_y(1:dim,11:end)*r_out';
        RR_temp(batch_j,:,:) = r_out*r_out';
    end
    r_end(:,  ((batch_i-1)*batch_size+1):(batch_i*batch_size) ) = r_end_temp;

    YR = YR + squeeze(sum(YR_temp,1));
    RR = RR + squeeze(sum(RR_temp,1));

    if print_progress_type == 2
        fprintf('%f of the training tps are done\n',batch_i/(tp_length/batch_size))
        toc
